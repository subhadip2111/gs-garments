...
const QuickViewModal: React.FC = () => {
  const { quickViewProduct, setQuickViewProduct, addToCart, user } = useApp();
  ...
  const handleAddToCart = () => {
    ...
    addToCart(quickViewProduct.id, selectedSize, selectedColor, quantity);
    handleClose();
  };
                  ...
                  {quickViewProduct.sizes.map(size => (
                    <button 
                      ...
                      className={`py-3 text-[10px] font-bold border transition-all ${
                        selectedSize === size ? 'bg-black text-white border-black shadow-lg' : 'border-gray-100 hover:border-black text-gray-400 hover:text-black'
                      }`}
                    >
                      {size}
                    </button>
